...
from config import (
    ...
    EXCHANGE_RATE_URL, NEWS_SOURCES, JOKE_API_URL
)
...
class APIClient:
    ...
    # === ФАКТЫ/ЮМОР: JokeAPI v2 ===
    async def fetch_joke(self) -> Optional[Dict[str, str]]:
        """Получить шутку или факт дня"""
        fallback_joke = {
            "setup": "Программист заходит в лифт. Хочет нажать 1-й этаж.",
            "punchline": "Но нажимает 0. Потому что для него всё с нуля начинается. 😄"
        }
        ...
        try:
            ...
            async with session.get(JOKE_API_URL) as resp:
                ...
                if data.get("error"):
                    logger.warning(f"JokeAPI error: {data}")
            ...
            logger.warning(f"Ошибка Joke API: {e}")
            return fallback_joke

    # === УТИЛИТА: Массовая загрузка ===
    async def fetch_all_data(self) -> Dict[str, Any]:
        """
        Загружает глобальные данные (крипта, фиат, новости, шутки) параллельно.
        ...
        data = {
            ...
            "joke": safe_extract(results[3], "joke"),
            "fetched_at": datetime.now(timezone.utc).isoformat()
        }

        # Логируем успешность
        success_count = sum(1 for v in [data["crypto"], data["fiat"], data["news"], data["joke"]] if v)
        logger.info(f"Fetched {success_count}/4 data sources successfully")
